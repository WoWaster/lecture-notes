...
\begin{corollary}
    Пусть $|K| = q$. Тогда $K$ -- поле разложения $X^q - X$ над любым подполем.
\end{corollary}
...
\begin{proof}
    Пусть $F$ -- поле разложения $X^q - X$ над $\F_p, \ q = p^n$. Проверим, что 
    у него нет кратных корней: $(X^q - X)' = qX^{q-1} -1 = -1 \Rightarrow$ в 
