...
\begin{proof}
    ...
    Проверим: $w_j = 0$ (аналогично $w_j = 0 \ \forall j)$.
    ...
    ${p_1}^{l_1} \ldots {p_{s-1}}^{l_{s-1}} (\mca) (w_j) = 0, \ j = 1, \ldots, s-1 \Rightarrow
        ...
        {p_s}^{l_s} \mid {p_1}^{l_1} \ldots {p_{s-1}}^{l_{s-1}} \Rightarrow ls = 0 \Rightarrow w_s = 0$.
\end{proof}

\begin{corollary}
    Пусть $\mca \in \End V$. Тогда $V = \bigoplus_{p|\chi_{\mca}} W$.
\end{corollary}

\begin{remark}
    $p \nmid \chi_{\mca} \Rightarrow W_p = 0$ (т.е. $\mu_{\mca, v} \mid \mu_{\mca} \mid \chi_{\mca}) \Rightarrow V = \bigoplus_p W_p$.
\end{remark}
