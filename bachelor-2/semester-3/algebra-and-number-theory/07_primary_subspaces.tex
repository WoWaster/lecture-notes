...
\begin{proposition}
    $p$ -- неприводимый, $W_p = \{w \in V \ | \ \mu_{\mca, w} = p^m, \ m \geq 0\}$.
    ...
    \begin{enumerate}
        ...
        \item $W_p$ -- $\mca$-инвариантно:
              \begin{center}
                  ...
                  $ \mu_{\mca, \mca w} | p^m \Rightarrow \mu_{\mca, \mca w} = p^l, \ l \leq m \Rightarrow \mca w \in W_p$
              \end{center}
...
\begin{proof}
    ...
    Переход: $f = gh, \ g = \varepsilon {p_1}^{r_1} \ldots {p_{s-1}}^{r_{s-1}}$, $h = {p_s}^{r_s}$, $(g, h) = 1 \Rightarrow
        V = V' \oplus V'', \ g(\mca |_{V'}) = 0, \ h(\mca |_{V''}) = 0$ (по предложению 7.2).

    По индукционному предположению, $V' =  \widetilde{W}_{p_1} \oplus \ldots \oplus \widetilde{W}_{p_{s-1}}$.
    ...
    $V = \widetilde{W}_{p_1} \oplus \ldots \oplus \widetilde{W}_{p_{s-1}} \oplus V'' \subset W_{p_1} + \ldots + W_{p_s}$.
...
\begin{corollary}
    Пусть $\mca \in \End V$. Тогда $V = \bigoplus_{p|\chi_{\mca}} W_p$, $p$ -- неприводимый приведеннный.
\end{corollary}
